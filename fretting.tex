...
In the previous chapter, I set forth several different methods in which we can measure ratios
of intervals to determine the quality of their temperament.  In this chapter, I will apply these
methods to different fretting schemes to detemine, from a mathematical standpoint, what kind of
tempered interval each fret represents.  In some cases, the fret will be a pure Pytagorean interval,
others will be meantone in nature or approximate equal temeperament, and others will be completely
unique.

The majority of fretting instructions published during this time provided the player with a practical
means of setting frets and did not dwell on theory.  Almost all of the sources have
very precise rules for determining fret placement with measurement tools such as a straight-edge
and compass.  These were common tools at the time for making geometrical divisions, and when
used for calculating fret placement, could be used to divide a line into any number of equal
parts.  The remainder of the sources are less precise and instruct the player to place the
fret somwhere near another until it sounds agreeable, or
to move a fret slightly in one direction or the other without providing any exact measurements.

What follows is a comparison of several fretting systems that
appeared in lute and vihuela treatises from the sixteenth and early seventeenth centuries.
For those sources that have instructions on placing frets using a compass, I have determined the
exact ratio of the interval using the arithmetic methods I outlined in the
previous chapter.  The details of the calculations can be found in the appendix at the end of this paper.

The tuning and temperament of the intervals found in these sources vary from Pythagorean, to meantone
and equal division, to invervals that do not conform to any kind of specific named temperament.
Methods on fret placement from the first half of the sixteenth century are almost exclusively
Pythagorean in nature.  Around the middle of the century, sources begin to introduce meantone
intervals into existing Pythagorean systems.  By the end of the sixteenth century,
newer sources present systems that
divide the octave into twelve approximately equal parts, but fall short of matching modern-day
equal temperament.  However, by the seventeenth century, we can find sources that provide instructions
for equal temperament that are the closest we have to modern-day equal temperament, yet they
did not appear to gain widespread use.

The overall picture for lute temperaments from the sixteenth through early seventeenth centuries
can be outlined as follows: Pythagorean tunings were used exclusively at the beginning of the period,
and retained some use for certain intervals later in the period.
By the mid-sixteenth century, lute players began to add different types of tempered intervals into
the existing Pythagorean scheme.  Some of these intervals were meantone, some
approximating equal temperament and others were solely original in their quality.
The important distinction between these systems and other keyboard
temperaments used at the time was that lute temperaments did not fall completely into one kind of
tuning or temperament. They were very much to themselves, unlike any other systems at the time.
...
It was probably around the time that fretted instruments began to incorporate
...
Fin\'{e}'s and it was mostly likely this frienship that resulted in
...
Another example of Pythagorean tuning appeared in a book published about the
...
In his assesment of thier fretting instructions, Marlk Lindley has concluded that
...
Successive instructions on fretting from the mid-sixteenth century onward de-emphasized Pythagorean
...
He uses a very straight-forward, step-by-step approach for setting each fret, begining with the
twelfth.
\begin{blocks}
...
% Gerle's first fret
Gerle's instructions continue with the first fret, and similar to his
...
For example, Gnassi opens his fourth chapter with the second fret, and uses the same
...
Once Gnassi has completed setting those frets, he does not revisit them.  However, this is not the case
...
% Gnassi's initial fourth fret
\input{figures/Gnassi-4}
...
% Gnassi's 6th fret
\input{figures/Gnassi-6}

% Gnassi's 8th fret
\input{figures/Gnassi-8}
...
Gnassi holds many similarities with other sources.  The second, fifth and
